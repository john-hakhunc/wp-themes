...
function adforest_search_params( $index, $second = '')
{
...
function adforest_display_cats( $pid )
{
		...
		{
			$cat = get_category( $c );
			$cats_html	.= '<span class="padding_cats"><a href="'.get_category_link( $cat->term_id ).'">'.esc_html( $cat->name ).'</a></span>';
		}
		return $cats_html;
...
function adforest_get_static_form($term_id = '', $post_id = '')
{
		...
		$vals[] = array(
				...
				'sub_title' 	=> $adforest_theme['sb_currency'] . " " .  __('only', 'adforest' ),
				'field_name' 	=> 'ad_price',
				'field_id' 		=> 'ad_price',
				'field_value' 	=> $price,
				'field_req' 	=> $required,
...
function adforest_video_icon()
{
	global $adforest_theme;
	if( isset( $adforest_theme['sb_video_icon'] ) && $adforest_theme['sb_video_icon'] && get_post_meta(get_the_ID(), '_adforest_ad_yvideo', true ) )
	{
		return '<a href="'.get_post_meta(get_the_ID(), '_adforest_ad_yvideo', true ).'" class="play-video"><i class="fa fa-play-circle-o"></i></a>';	
	}	
}
}