		   ...
		   <div class="search-result-item-body">
			  <div class="row">
				 <div class="col-md-5 col-sm-12 col-xs-12">
					...
					'.$package_type_html.'
					<span class="label label-success sb_user_type">'.get_user_meta($this->user_info->ID, '_sb_user_type', true ).'</span>
					'.$badge.'
					'.$rating .'
		   ...
		   <div class="tab-content">
			  <div class="profile-edit tab-pane fade in active" id="profile">
				 ...
				 <dl class="dl-horizontal">
					...
					<dd>
					   '.esc_attr( get_user_meta( $this->user_info->ID, '_sb_address', true ) ).'
					</dd>
						'.$paid_html.'
				 </dl>
			  </div>
...
function adforest_my_inactive_ads()
{
	...
	$ads = new ads();
	echo ( $ads->adforest_get_ads_grid_inactive( $args, $paged, $show_pagination) );
...
function adforest_profile_update_ajax_processed()
{
	...
	$profile	= new adforest_profile();
	$uid	=	$profile->user_info->ID;
	wp_update_user( array( 'ID' => $uid, 'display_name' => $params['sb_user_name'] ) );
	update_user_meta($uid, '_sb_contact', $params['sb_user_contact']);
	update_user_meta($uid, '_sb_address', $params['sb_user_address']);
...
{
	global $wpdb;
	$rows = $wpdb->get_results( "SELECT ID FROM $wpdb->posts WHERE post_author = '$user_id' AND post_status = 'pending'" );
	...
	else
	{
		...
		if( get_user_meta( $uid, '_sb_expire_ads', true ) != '-1' )
		{
			if( get_user_meta( $uid, '_sb_expire_ads', true ) < date('Y-m-d') )
			{
				...
				adforest_redirect_with_msg( get_the_permalink( $adforest_theme['sb_packages_page'] ), __( "Your package has bee expired.", 'adforest') );
				exit;		
			}
		}	
	}
		...
		foreach( $notes as $note )
		{
			...
			if( count( $media ) > 0 )
			{
				...
				foreach( $media as $m )
				{
					if( $counting > 1 )
					{
						$image  = wp_get_attachment_image_src( $m->ID, 'adforest-single-small');
						if( $image[0] != "" )
						{
							$ad_img = $image[0];	
						}
...
function adforest_submit_bid()
{
	...
	$params = array();
    parse_str($_POST['sb_data'], $params);
	$comments	=	$params['bid_comment'];
	$offer	=	$params['bid_amount'];
	$ad_id	=	$params['ad_id'];
...
function adforest_bidding_stats($ad_id)
{
	...
	if($total_bids > 0)
	{
		...
		if( isset($adforest_theme['sb_price_direction']) && $adforest_theme['sb_price_direction'] == 'right' )
		{
			$max =  $max . '<small>' . $adforest_theme['sb_currency'] . '</small>';
			$min =  $min . '<small>' . $adforest_theme['sb_currency'] . '</small>';
		}	
		else if( isset($adforest_theme['sb_price_direction']) && $adforest_theme['sb_price_direction'] == 'left' )
		{
			$max =  '<small>' . $adforest_theme['sb_currency'] . '</small>' . $max;
			$min =  '<small>' . $adforest_theme['sb_currency'] . '</small>' . $min;
		}	
		else
		{
			$max =  '<small>' . $adforest_theme['sb_currency'] . '</small>' . $max;
