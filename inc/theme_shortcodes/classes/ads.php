...
function adforest_get_ads_grid($args, $paged, $show_pagination = 0, $fav_ads )
{
	...
	if( isset( $adforest_theme['sb_rtl'] ) && $adforest_theme['sb_rtl'] )
	{
		$flip_it = 'flip';
	...
	if ( $ads->have_posts() ) {
		...
		$edit	=	'<li><a data-toggle="tooltip" data-placement="top" title="" data-original-title="'.__('Edit this Ad', 'adforest' ) .'" href="'.get_the_permalink( $adforest_theme['sb_post_ad_page'] ) . '?id=' . get_the_ID() .'"><i class="fa fa-pencil edit"></i></a> </li>';
			...
			{
			$delete	=	'<li>
									   <a  href="javascript:void(0);" data-adid="'.get_the_ID().'" class="remove_ad" data-toggle="confirmation" data-singleton="true" data-title="'.__('Are you sure?', 'adforest' ) .'" data-content=""  >
									   <i class="fa fa-times delete"></i>
									   </a>
									   </li>';
			}
		...
		else
		{
			...
			{
				$remove	=	'<li style="margin-right:39px;">
								   <a  data-toggle="confirmation" data-singleton="true" data-title="'.__('Are you sure?', 'adforest' ) .'" data-content="" href="javascript:void(0);" data-adid="'.get_the_ID().'" class="remove_fav_ad" >
								   <i class="fa fa-times delete"></i>
								   </a>
								   </li><li></li>';
			}
...
function adforest_get_ads_grid_slider($args, $title, $col = 6 )
{
	...
	if ( $ads->have_posts() ) {
		...
		return '<div class="col-xs-12 col-md-12 col-sm-12 margin-bottom-30"><div class="grid-card">
                     ...
                     <div class="col-md-4 col-sm-5 col-xs-12">
                        <div class="message-inbox">
                           <div class="message-header">
							  ...
							  <span ><a class="messages_actions active" sb_action="received_msgs_ads_list"><small>'.__('Recevied Offers','adforest').'</small></a></span>
	...
	if ( $ads->have_posts() )
	{
		...
		while ( $ads->have_posts() )
		{
			...
			if( count( $media ) > 0 )
			{
				foreach( $media as $m )
				{
					$img  = wp_get_attachment_image_src( $m->ID, 'adforest-ad-related');
					$ad_img	=	$img[0];
					break;
				}
...
function adforest_search_layout_list( $pid, $col = 12 )
{
    ...
    foreach($post_categories as $c)
    {
		...
		if( $it_one != 1 )
			$cls	=	'padding-left';
    $cat = get_category( $c );
    $cats_html	.= '<span><a class="'.$cls.'" href="'.get_category_link( $cat->term_id ).'">'.esc_html( $cat->name ).'</a></span>';
		$it_one++;
    }
	$output .= '
	<div class="category-title">
            ...
            <div class="category-grid-box-1">
			   ...
			   '.$is_feature.'
                  ...
                  <ul class="ad-meta-info">
					 '.$warranty.'
					 '.$condition_html.'
                     <li> <a href="javascript:void(0);"><i class="flaticon-phone-call"></i>
					 '.get_post_meta(get_the_ID(), '_adforest_poster_contact', true ).'
					 </a>
					 </li>
                  </ul>
...
function adforest_search_layout_list_2( $pid )
{
		$number	= 0;
		global $adforest_theme;
		$cats_html	=	adforest_display_cats( $pid );
