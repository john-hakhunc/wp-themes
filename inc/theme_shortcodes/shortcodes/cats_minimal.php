	...
	if( isset( $atts['cats'] ) )
	{
		...
		if( count( $rows ) > 0 )
		{
			foreach($rows as $row )
			{
				...
				{
					$category = get_category($row['cat']);
					if( count( $category ) == 0 )
						continue;
					$count = $category->category_count;
					
					
					$categories_html .= '<li class="col-lg-3 col-md-3 col-sm-6 col-xs-12">
                              <a href="'. adforest_cat_link_page($row['cat'], $cat_link_page) .'">'.$category->name.'<span>('.$count. ' ' . __('Ads', 'adforest' ) .')</span>
                              <i class="'.$row['icon'].'"></i>
                              </a> 
                           </li>';
				}
